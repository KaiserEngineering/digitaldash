...
<div class="col-11">
  ...
  <select id="logName" bind:value={current} on:change="{e => loadContent( e.target.value )}" class="custom-select form-control d-block w-100 mb-2" required>
    {#each logNames as log}
      <option value={log}>{log}</option>
    {/each}
  </select>

  <button type="button" class="mb-2 btn btn-secondary" on:click="{refresh}">Refresh</button>
